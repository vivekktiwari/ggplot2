...
#'
#' @param title A character string or expression indicating a title of a_guide.
...
#' @param title.a_theme A theme object for rendering the title text. Usually the
#'   object of \code{\link{a_element_text}} is expected. By default, the theme is
#'   specified by \code{legend.title} in \code{\link{a_theme}} or theme.
...
#'   label. One of "top", "bottom" (default for horizontal a_guide), "left", or
#'   "right" (default for vertical a_guide).
#' @param a_label.a_theme A theme object for rendering the label text. Usually the
#'   object of \code{\link{a_element_text}} is expected. By default, the theme is
#'   specified by \code{legend.text} in \code{\link{a_theme}} or theme.
#' @param a_label.hjust A numeric specifying horizontal justification of the
#'   label text.
#' @param a_label.vjust A numeric specifying vertical justification of the label
#'   text.
...
#'   \code{legend.key.size} in \code{\link{a_theme}} or theme.
...
a_guide_geom.legend <- function(a_guide, layers, default_mapping) {
  ...
  a_guide$geoms <- plyr::llply(layers, function(a_layer) {
    ...
    # override.a_aes in guide_legend manually changes the geom
  ...
  # Finally, remove this a_guide if no a_layer is drawn
...
a_guide_gengrob.legend <- function(a_guide, a_theme) {
  ...
  if (!a_guide$a_label || is.null(a_guide$key$.a_label)) {
    ...
    # a_label.a_theme in param of guide_legend() > a_theme$legend.text.align > default
  ...
  # layout of key-a_label depends on the direction of the guide
