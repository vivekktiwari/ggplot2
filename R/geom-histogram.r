...
#' # raw data. The bins have constant width on the original a_scale.
#'
#' # Using log scales does not work here, because the first
#' # bar is anchored at zero, and so whens transformed becomes negative
#' # infinity. This is not a problem when transforming the scales, because
#' # no observations have 0 ratings.
#' m + a_geom_histogram(origin = 0) + ggplot2Animint:::a_coord_trans(x = "log10")
#' # Use origin = 0, to make sure we don't take sqrt of negative values
#' m + a_geom_histogram(origin = 0) + ggplot2Animint:::a_coord_trans(x = "sqrt")
#'
