...
#'
#' Use \code{a_theme_get} to get the current theme, and \code{a_theme_set} to
...
#' theme. \code{a_theme_replace} will completely replace the element, so any
...
#'   the one returned \code{by a_theme_grey()}. Complete themes behave
...
add_a_theme <- function(t1, t2, t2name) {
  ...
  for (item in names(t2)) {
    ...
    if (is.null(x) || inherits(x, "a_element_blank")) {
      # If x is NULL or a_element_blank, then just assign it y
      x <- y
    } else if (is.null(y) || is.character(y) || is.numeric(y) ||
               is.logical(y) || inherits(y, "a_element_blank")) {
      # If y is NULL, or a string or numeric vector, or is a_element_blank, just replace x
      x <- y
    } else {
...
#' @export
update_a_theme <- function(oldtheme, newtheme) {
  ...
  oldtheme[newitem_names] <- a_theme_get()[newitem_names]
  ...
  oldtheme <- do.call(a_theme, c(oldtheme,
...
#' @param a_theme A theme object (like a_theme_grey())
