...
#' p + ggplot2Animint:::a_facet_grid(. ~ cyl, labeller = a_label_value)
#'
#' \donttest{
...
#' p + facet_grid(. ~ cyl2)
#' p + facet_grid(. ~ cyl2, labeller = a_label_parsed)
#' p + facet_wrap(~vs + cyl2, labeller = a_label_parsed)
#' }
NULL
...
#' p <- a_plot(mtcars, a_aes(wt, mpg)) + a_geom_point()
#' p + ggplot2Animint:::a_facet_grid(vs ~ ., labeller = a_label_bquote(alpha ^ .(vs)))
#' p + ggplot2Animint:::a_facet_grid(. ~ vs, labeller = a_label_bquote(cols = .(vs) ^ .(vs)))
#' p + ggplot2Animint:::a_facet_grid(. ~ vs + am, labeller = a_label_bquote(cols = .(am) ^ .(vs)))
