...
a_ScalesList <- a_ggproto("a_ScalesList", NULL,
  ...
  add = function(self, a_scale) {
    ...
    # Remove old scale for this a_aesthetic (if it exists)
...
# @param a_aesthetics A list of aesthetic-variable mappings. The name of each
...
scales_add_defaults <- function(scales, data, a_aesthetics, env) {
  ...
  # No new a_aesthetics, so no new scales to add
...
# @param a_aesthetics A character vector of a_aesthetics. Typically c("x", "y").
scales_add_missing <- function(plot, a_aesthetics, env) {

  # Keep only a_aesthetics that aren't already in plot$scales
  a_aesthetics <- setdiff(a_aesthetics, plot$scales$input())

  for (a_aes in a_aesthetics) {
    a_scale_name <- paste("a_scale", a_aes, "continuous", sep = "_")
