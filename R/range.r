...
a_RangeDiscrete <- ggproto("a_RangeDiscrete", a_Range,
  train = function(self, x, drop = FALSE) {
    self$range <- scales::train_discrete(x, self$range, drop)
  }
)

a_RangeContinuous <- ggproto("a_RangeContinuous", a_Range,
...
continuous_range <- function() {
  ggproto(NULL, a_RangeContinuous)
}

discrete_range <- function() {
  ggproto(NULL, a_RangeDiscrete)
}
