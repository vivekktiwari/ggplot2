...
#'
#' # Set order for multiple a_guides
...
build_guides <- function(scales, layers, default_mapping, a_position, a_theme, a_guides, a_labels) {
  ...
  # size of key (also used for bar in colorbar a_guide)
...
validate_guide <- function(a_guide) {
  # if a_guide is specified by character, then find the corresponding guide
...
a_guides_train <- function(scales, a_theme, a_guides, a_labels) {
  ...
  for (a_scale in scales$scales) {
    ...
    # which is prior to scale_ZZZ(a_guide=XXX)
    # a_guide is determined in order of:
    #   + a_guides(XXX) > + scale_ZZZ(a_guide=XXX) > default(i.e., legend)
    ...
    # check the consistency of the guide and a_scale.
    ...
    # each a_guide object trains scale within the object,
...
a_guides_build <- function(ggrobs, a_theme) {
  ...
  if (a_theme$legend.box == "horizontal") {
    ...
    # add space between the a_guide-boxes
  ...
  # add margins around the a_guide-boxes.
