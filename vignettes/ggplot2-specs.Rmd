...
*   An __integer__ or __name__: 0 = blank, 1 = solid, 2 = dashed, 3 = dotted, 
    ...
    a_plot(linetypes, a_aes(0, y)) + 
      a_geom_segment(a_aes(xend = 5, yend = y, linetype = lty)) + 
      a_scale_linetype_identity() + 
      a_geom_text(a_aes(a_label = lty), hjust = 0, nudge_y = 0.2) +
      a_scale_x_continuous(NULL, breaks = NULL) + 
...
*   An __integer__ in $[0, 25]$:
    ...
    a_plot(shapes, a_aes(x, y)) + 
      a_geom_point(a_aes(shape = shape), size = 5, fill = "red") +
      a_geom_text(a_aes(a_label = shape), hjust = 0, nudge_x = 0.15) +
      a_scale_shape_identity() +
      ggplot2Animint:::expand_limits(x = 4.1) +
      a_scale_x_continuous(NULL, breaks = NULL) + 
      a_scale_y_continuous(NULL, breaks = NULL)
    ```
...
a_plot(sizes, a_aes(size, stroke, size = size, stroke = stroke)) + 
  a_geom_abline(slope = -1, intercept = 6, colour = "white", size = 6) + 
  a_geom_point(shape = 21, fill = "red") +
...
a_plot(df, a_aes(x, y)) + 
  a_geom_text(a_aes(a_label = family, family = family))
```
...
just$a_label <- paste0(just$hjust, ", ", just$vjust)

a_plot(just, a_aes(hjust, vjust)) +
  a_geom_point(colour = "grey70", size = 5) + 
  a_geom_text(a_aes(a_label = a_label, hjust = hjust, vjust = vjust))
```
