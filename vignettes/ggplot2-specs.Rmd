...
*   An __integer__ or __name__: 0 = blank, 1 = solid, 2 = dashed, 3 = dotted, 
    ...
    a_plot(linetypes, aes(0, y)) + 
      ...
      a_geom_text(aes(label = lty), hjust = 0, nudge_y = 0.2) +
...
*   An __integer__ in $[0, 25]$:
    ...
    a_plot(shapes, aes(x, y)) + 
      ...
      a_geom_text(aes(label = shape), hjust = 0, nudge_x = 0.15) +
...
a_plot(df, aes(x, y)) + 
  a_geom_text(aes(label = family, family = family))
```
...
a_plot(just, aes(hjust, vjust)) +
  ...
  a_geom_text(aes(label = label, hjust = hjust, vjust = vjust))
```
