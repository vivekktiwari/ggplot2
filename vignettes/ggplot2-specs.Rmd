...
*   An __integer__ or __name__: 0 = blank, 1 = solid, 2 = dashed, 3 = dotted, 
    ...
    a_plot(linetypes, aes(0, y)) + 
      a_geom_segment(aes(xend = 5, yend = y, linetype = lty)) + 
      a_scale_linetype_identity() + 
      a_geom_text(aes(a_label = lty), hjust = 0, nudge_y = 0.2) +
...
*   An __integer__ in $[0, 25]$:
    ...
    a_plot(shapes, aes(x, y)) + 
      a_geom_point(aes(shape = shape), size = 5, fill = "red") +
      a_geom_text(aes(a_label = shape), hjust = 0, nudge_x = 0.15) +
...
a_plot(sizes, aes(size, stroke, size = size, stroke = stroke)) + 
...
a_plot(df, aes(x, y)) + 
  a_geom_text(aes(a_label = family, family = family))
```
...
a_plot(just, aes(hjust, vjust)) +
  a_geom_point(colour = "grey70", size = 5) + 
  a_geom_text(aes(a_label = a_label, hjust = hjust, vjust = vjust))
```
