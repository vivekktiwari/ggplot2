...
test_that("facets split up the data", {
  l1 <- a_plot(df, aes(x, y)) + geom_point() + facet_wrap(~z)
  l2 <- a_plot(df, aes(x, y)) + geom_point() + facet_grid(. ~ z)
  l3 <- a_plot(df, aes(x, y)) + geom_point() + facet_grid(z ~ .)
...
test_that("shrink parameter affects scaling", {
  ...
  l2 <- a_plot(df, aes(1, y)) + stat_summary(fun.y = "mean")
  r2 <- pranges(l2)
  expect_equal(r2$y[[1]], c(2, 2))

  l3 <- a_plot(df, aes(1, y)) + stat_summary(fun.y = "mean") +
