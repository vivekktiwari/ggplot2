...
test_that("layers with empty data are silently omitted", {
  ...
  d <- a_plot(df0, aes(mpg,wt)) + a_geom_point()
  expect_equal(nrow(layer_data(d)), 0)

  d <- a_plot() + a_geom_point(data = df0, aes(mpg,wt))
  expect_equal(nrow(layer_data(d)), 0)

  # Regular mtcars data, x=mpg, y=wt, normal points and points from empty data frame
  d <- a_plot(mtcars, aes(mpg, wt)) + a_geom_point() + a_geom_point(data = df0)
  ...
  d <- a_plot(mtcars, aes(mpg, wt)) + a_geom_point(data = df0)
  expect_equal(nrow(layer_data(d, 1)), 0)
})


test_that("plots with empty data and vectors for aesthetics work", {
  d <- a_plot(NULL, aes(1:5, 1:5)) + a_geom_point()
  expect_equal(nrow(layer_data(d)), 5)

  d <- a_plot(data.frame(), aes(1:5, 1:5)) + a_geom_point()
  expect_equal(nrow(layer_data(d)), 5)

  d <- a_plot() + a_geom_point(aes(1:5, 1:5))
  expect_equal(nrow(layer_data(d)), 5)
})
...
test_that("missing layers generate one grob per panel", {
  ...
  base <- a_plot(df, aes(x, y)) + a_geom_point(shape = NA, na.rm = TRUE)
