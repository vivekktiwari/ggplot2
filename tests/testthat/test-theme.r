...
test_that("Modifying theme element properties with + operator", {
  ...
  t <- theme_grey() + theme(axis.title.x = a_element_text(colour = 'red', margin = margin()))
...
test_that("Replacing theme elements with %+replace% operator", {
  # Changing a "leaf node" works
  t <- theme_grey() %+replace% theme(axis.title.x = a_element_text(colour = 'red'))
  ...
  t <- theme_grey() %+replace% theme(axis.title = a_element_text(colour = 'red'))
  expect_identical(t$axis.title, a_element_text(colour = 'red'))
  # Descendent is unchanged
  expect_identical(t$axis.title.x, theme_grey()$axis.title.x)
...
test_that("Calculating theme element inheritance", {
  ...
  t <- theme_grey(base_size = 12) +
    theme(axis.title   = a_element_text(size = rel(0.5))) +
    theme(axis.title.x = a_element_text(size = rel(0.5)))
  ...
  t <- theme_grey() + theme(text = a_element_blank())
...
test_that("Complete and non-complete themes interact correctly with each other", {
  ...
  t <- theme_bw() + theme(text = a_element_text(colour = 'red'))
  ...
  t <- theme(text = a_element_text(colour = 'red')) + theme_bw()
  expect_true(attr(t, "complete"))
  ...
  t <- theme(text = a_element_text(colour = 'blue')) +
    theme(text = a_element_text(colour = 'red'))
...
test_that("Complete and non-complete themes interact correctly with ggplot objects", {
  ...
  p <- a_plot_build(qplot(1:3, 1:3) + theme_bw() + theme(text = a_element_text(colour = 'red')))
  expect_true(attr(p$plot$theme, "complete"))

  # Compare the theme objects, after sorting the items, because item order can differ
  pt <- p$plot$theme
  tt <- theme_bw() + theme(text = a_element_text(colour = 'red'))
  pt <- pt[order(names(pt))]
  tt <- tt[order(names(tt))]
  expect_identical(pt, tt)


  p <- a_plot_build(qplot(1:3, 1:3) + theme(text = a_element_text(colour = 'red')) + theme_bw())
  expect_true(attr(p$plot$theme, "complete"))
  # Compare the theme objects, after sorting the items, because item order can differ
  pt <- p$plot$theme
  tt <- theme(text = a_element_text(colour = 'red')) + theme_bw()
  pt <- pt[order(names(pt))]
  tt <- tt[order(names(tt))]
  expect_identical(pt, tt)


  p <- a_plot_build(qplot(1:3, 1:3) + theme(text = a_element_text(colour = 'red', face = 'italic')))
  ...
  p <- a_plot_build(qplot(1:3, 1:3) +
    theme(text = a_element_text(colour = 'red')) +
    theme(text = a_element_text(face = 'italic')))
  expect_false(attr(p$plot$theme, "complete"))
  expect_equal(p$plot$theme$text$colour, "red")
  expect_equal(p$plot$theme$text$face, "italic")
  ...
  p <- a_plot_build(qplot(1:3, 1:3) +
    theme(text = a_element_text(colour = 'red'), text = a_element_text(face = 'italic')))
  expect_false(attr(p$plot$theme, "complete"))
  expect_equal(p$plot$theme$text$colour, "red")
...
test_that("theme(validate=FALSE) means do not validate_element", {
  p <- qplot(1:3, 1:3)
  bw <- p + theme_bw()
  red.text <- theme(text = a_element_text(colour = "red"))
