...
test_that("names of values used in manual scales", {
   s <- ggplot2Animint:::a_scale_colour_manual(values = c("8" = "c","4" = "a","6" = "b"))
   s$train(c("4", "6", "8"))
...
test_that("insufficient values raise an error", {
  ...
  expect_error(a_plot_build(p + ggplot2Animint:::a_scale_colour_manual(values = "black")),
    "Insufficient values")

  # Should be sufficient
  a_plot_build(p + ggplot2Animint:::a_scale_colour_manual(values = c("black", "black")))

})

test_that("values are matched when scale contains more unique valuesthan are in the data", {
  s <- ggplot2Animint:::a_scale_colour_manual(values = c("8" = "c", "4" = "a",
    "22" = "d", "6"  = "b"))
  s$train(c("4", "6", "8"))
  expect_equal(s$map(c("4", "6", "8")), c("a", "b", "c"))
})
