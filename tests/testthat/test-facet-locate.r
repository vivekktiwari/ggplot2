...
test_that("grid: facet order follows default data frame order", {
  ...
  # CBA for rows 1:3
  # cba for cols 1:3
  lay <- get_layout(a_plot(d, aes(x, y)) + facet_grid(fy ~ fx) + geom_point())
  ...
  # CBA for rows 1:3
  # cba for cols 1:3
  lay <- get_layout(a_plot(d, aes(x, y)) + facet_grid(fy ~ fx) +
  ...
  # BCA for rows 1:3
  # acb for cols 1:3
  lay <- get_layout(a_plot(mapping = aes(x, y)) + facet_grid(fy ~ fx) +
...
test_that("wrap: facet order follows default data frame order", {
  # Facets should be in order:
  # cba for panels 1:3
  lay <- get_layout(a_plot(d, aes(x, y)) + facet_wrap(~fx) + geom_point())
  expect_equal(as.character(lay$fx), c("c","b","a")[lay$PANEL])

  # When adding d2, facets should still be in order:
  # cba for panels 1:3
  lay <- get_layout(a_plot(d, aes(x, y)) + facet_wrap(~fx) +
    geom_blank(data = d2) + geom_point())
  expect_equal(as.character(lay$fx), c("c","b","a")[lay$PANEL])

  # With no default data: should search each layer in order
  # acb for panels 1:3
  lay <- get_layout(a_plot(mapping = aes(x, y)) + facet_wrap(~fx) +
    geom_blank(data = d2) + geom_point(data = d))
